...
javascript runtimes can model/compile/jit/execute their js code.
...
Also the real world calls RAM indexes pointers. I'm
...
the normal industry nomenclature would probably be `C struct` (with some caveats around packing, padding, field ordering, etc). As one
last thing: I'm pretending in this example that our computers have byte sized words and that ASCII is great because
32bit/64bit and utf8/utf16 don't add anything to this post and we'd have to count in multiples of 4 or 8. Finally
my hashtables aren't fast at all.


## More notation: how computers **address** RAM {#more-notation-how-computers-address-ram}
...
it can directly write machine code that uses those offsets. Let's write some psuedocode for returning the age of the 2nd person:
...
strings can't be packed into the same orderly layout as above where each field starts at a fixed offset. We typically store the strings somewhere else and store the RAM index of that location in the person record. Let's draw that out. We because these
...
sized data". So now you have enough background to understand a really basic record/object type. This is an idealized example, in the real world the actual layout can be more complicated but
for our purposes today this is a good enough mental model of "how to layout an object in memory for maximum speed".
...
The simplest method is to put all the values right next to each other one after another and use `\0` to separate them.
...
keys to numbers. Then we convert those numbers to a bucket index. So I might have 10 buckets and when I store "foo" I run a function which says hash("foo") = some random number like 12313213
and then I convert that to a bucket index by saying `12313213%10 = 3`. Then in bucket number 3 I store something like: `foo\0value\0`. The great thing about this
